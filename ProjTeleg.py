        ...
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            if label.split(":")[0].lower() == 'person' and smsSent == False:
                ...
                if humidity is not None and temperature is not None and soil_temperature is not None:
                    ...
                    Thread(target= prepare_json(temperature, humidity, soil_temperature, intruder = intruder)).start()
                else:
                ...
                if humidity is not None and temperature is not None and soil_temperature is not None:
                    ...
                    Thread(target= prepare_json(temperature, humidity, soil_temperature, intruder = None)).start()
                else:
                    print("Failed to get reading. Try again!")
