...
#______________________________________SIM800 Area____________________________________________________________#
...
def upload_to_server(sendToServer, url):    
    ...
    time.sleep(3)
    ser.write(b"AT+HTTPPARA=URL," + url.encode() +b"\r\n")
    time.sleep(3)
    ser.write(b"AT+HTTPPARA=\"CONTENT\",\"application/json\""+b"\r\n")
    time.sleep(3)
    ser.write(b"AT+HTTPDATA=" + str(len(sendToServer)).encode() + b",100000"+b"\r\n")
    time.sleep(3)
    ser.write(sendToServer.encode()+b"\r\n")
    time.sleep(3)
...
def prepare_json(temp, hum, soil, intruder = None):
    ...
    url = "https://energy-api.milsat.africa/api/FarmStatus/UpdateFarmStatus"
        ...
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            if label.split(":")[0].lower() == 'person' and smsSent == False:
                ...
                if humidity is not None and temperature is not None and soil_temperature is not None:
                    ...
                    prepare_json(temperature, humidity, soil_temperature, intruder = intruder)
                else:
